...
for domain in "${domains[@]}"; do
  ...
  echo "ssl_cert = </var/mail/ssl/certs/$domain/cert.crt" >> /etc/dovecot/conf.d/10-ssl.conf
  echo "ssl_key = </var/mail/ssl/certs/$domain/private.key" >> /etc/dovecot/conf.d/10-ssl.conf
  echo "}" >> /etc/dovecot/conf.d/10-ssl.conf
